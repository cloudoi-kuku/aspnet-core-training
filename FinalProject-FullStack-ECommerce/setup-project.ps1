...
services:
  ...
  frontend:
    ...
    ports:
      - "3000:3000"
    environment:
      - NEXT_PUBLIC_API_URL=http://backend:5050
    depends_on:
      - backend
    networks:
      - ecommerce-network
...
Write-Host "     - Backend API: http://localhost:5050" -ForegroundColor Gray
Write-Host "     - API Docs: http://localhost:5050/swagger" -ForegroundColor Gray
Write-Host ""
